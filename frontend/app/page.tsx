...
import { Bell, BookOpen, BriefcaseBusiness, Building2, ChevronDown, CircleUserRound, Code2, LayoutDashboard, Menu, Moon, Search, Settings, Sparkles, Target, Trophy, Users, X } from "lucide-react";
...
export default function Home() {
  ...
  return <main className={`${dark ? "bg-[#090b16] text-white" : "bg-slate-100 text-slate-950"} min-h-screen transition-colors`}>
    ...
    <aside className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} fixed z-20 flex h-screen w-72 flex-col overflow-y-auto bg-[#0d1020] p-6 text-white transition-transform lg:translate-x-0`}>
      ...
      <nav className="min-h-0 flex-1 space-y-2">{navItems.map((item) => <button key={item.label} onClick={() => { const routes: Record<string, string> = { "My learning": "/learning", Opportunities: "/opportunities", Companies: "/companies", "Skill intelligence": "/ai", "Industry intelligence": "/industry", "Student success": "/success", "Skill bridge": "/bridge", "Code lab": "/editor", Community: "/community" }; const route = routes[item.label]; if (route) window.location.href = route; else showMessage("You are already on Overview"); }} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm ${item.label === "Overview" ? "bg-violet-500/15 text-violet-200" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><item.icon size={18}/>{item.label}</button>)}</nav>
      <div className="mt-6 shrink-0 space-y-2 border-t border-white/10 pt-4"><button onClick={() => showMessage("Settings are saved for Module 3")} className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-slate-400"><Settings size={18}/> Settings</button><div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-violet-400 to-cyan-400 font-bold text-slate-950">AK</div><div><p className="text-sm">Aarav Kapoor</p><p className="text-xs text-slate-500">Student</p></div><ChevronDown size={16} className="ml-auto text-slate-500"/></div></div>
    </aside>
